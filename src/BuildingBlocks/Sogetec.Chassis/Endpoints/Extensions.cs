...
public static class EndpointExtensions
{
    ...
    {
        public IHostApplicationBuilder AddEndpoints(List<Assembly> modules)
        {
            var types = modules.SelectMany(a =>
                a.DefinedTypes.Where(type =>
                    type is { IsAbstract: false, IsInterface: false } && type.IsAssignableTo(typeof(IEndpoint))
                )
            );
            var serviceDescriptors = types.Select(type => ServiceDescriptor.Transient(typeof(IEndpoint), type)).ToArray();
